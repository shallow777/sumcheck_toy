...
impl Transcript {
    ...
    pub fn append_field<F:PrimField>(&mut self, label: &'static [u8], x: &F) {
        let mut buf = Vec::new();
        x.serialize_compressed(&mut buf).expect("serialize");
        self.append_bytes(label, &buf);
    }

    pub fn challenge_scalar<F:PrimField>(&mut self, label: &'static [u8]) -> F {
        ...
        let out = fork.finalize_fixed_reset().expect("finalize");
